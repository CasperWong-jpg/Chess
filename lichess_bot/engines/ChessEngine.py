...
class ChessEngine(object):
    ...
    def promotePawn(board, row, col, AIMode, simplePiece):
        counter = 0
        while counter < 3:
            if AIMode:
                promotedPiece = 'q'
            elif simplePiece != None:
                try:
                    promotedPiece = simplePiece
                except:
                    promotedPiece = 'q'
            else:  # Animations mode
                promotedPiece = simpledialog.askstring('getUserInput', 
            "What piece would you like to promote to? ([q]ueen, [r]ook, [b]ishop, k[n]ight): ")
            if promotedPiece.strip().lower() == 'q':
                board[row][col] = Queen(board[row][col].color)
                return
            if promotedPiece.strip().lower() == 'r':
                board[row][col] = Rook(board[row][col].color)
                return
            if promotedPiece.strip().lower() == 'b':
                board[row][col] = Bishop(board[row][col].color)
                return
            if promotedPiece.strip().lower() == 'n':
    ...
    @staticmethod
    def specialRules(board, row, col, AIMove = False, simplePiece = None):
        if isinstance(board[row][col], King) and not board[row][col].hasMoved and col == 2:
            ...
            ChessEngine.promotePawn(board, row, col, AIMove, simplePiece)    # Pawn promotion
    ...
    @staticmethod
    def AIMove(board, color, depth, bot = False):
        # Find best move
        if color == 'b':
        ...
        ChessEngine.specialRules(board, nextRow, nextCol, True)
